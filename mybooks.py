# creaing the application GUI
from tkinter import Tk,Button,Scrollbar,Listbox,StringVar,Entry,W,E,N,S,END
...
def on_closing():
    dd = db
    if messagebox.askokcancel("Quit","Do you want to quite?"):
        root.destroy()
        del dd
...
root.geometry("850x500") #set a size for application window
root.resizable(width=False,height=False) #Prevents the application window from resizing

# Creating  Labels and entry widgets
title_label = ttk.Label(root,text= "Title", background  ="light blue", font =("TkDefaultFont",18))
...
# add a buttons to insert input into database
add_btn = Button(root, text="Add Book", bg="#333", fg="white", font="helvetica 10 bold",command=add_book)
...
list_box.configure(yscrollcommand=scroll_bar.set) #Enable vertical scrolling
scroll_bar.configure(command=list_box.yview)

# Add more Buttton Widgets
